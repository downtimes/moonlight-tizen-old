...
void MoonlightInstance::OnConnectionStopped(uint32_t error) {
  ...
  ClearInputEventRequest(PP_INPUTEVENT_CLASS_MOUSE | PP_INPUTEVENT_CLASS_WHEEL |
  ...
  pp::Var response(std::string(MSG_STREAM_TERMINATED) +
...
void MoonlightInstance::HandleMessage(const pp::Var& var_message) {
  ...
  } else if (strcmp(method.c_str(), MSG_HTTPINIT) == 0) {
    NvHTTPInit(callbackId, params);
  } else if (strcmp(method.c_str(), MSG_MAKE_CERT) == 0) {
    MakeCert(callbackId, params);
  } else if (strcmp(method.c_str(), MSG_PAIR) == 0) {
    HandlePair(callbackId, params);
  } else if (strcmp(method.c_str(), MSG_STUN) == 0) {
...
// TODO: The data doesn't line up with what we get from the JS side.
void MoonlightInstance::HandleStartStream(int32_t callbackId,
                                          pp::VarArray args) {
...
void MoonlightInstance::HandleStopStream(int32_t callbackId,
                                         pp::VarArray args) {
...
void MoonlightInstance::HandleOpenURL(int32_t callbackId, pp::VarArray args) {
  m_HttpThreadPool[m_HttpThreadPoolSequence++ % HTTP_HANDLER_THREADS]
      ->message_loop()
      .PostWork(m_CallbackFactory.NewCallback(&MoonlightInstance::NvHTTPRequest,
                                              callbackId, args));
}
...
void MoonlightInstance::HandlePair(int32_t callbackId, pp::VarArray args) {
  m_HttpThreadPool[m_HttpThreadPoolSequence++ % HTTP_HANDLER_THREADS]
      ->message_loop()
      .PostWork(m_CallbackFactory.NewCallback(&MoonlightInstance::PairCallback,
                                              callbackId, args));
}

void MoonlightInstance::PairCallback(int32_t /*result*/, int32_t callbackId,
                                     pp::VarArray args) {
  char* ppkstr;
  int err = gs_pair(atoi(args.Get(0).AsString().c_str()),
                    args.Get(1).AsString().c_str(),
                    args.Get(2).AsString().c_str(), &ppkstr);
...
void MoonlightInstance::HandleSTUN(int32_t callbackId, pp::VarArray args) {
  m_HttpThreadPool[m_HttpThreadPoolSequence++ % HTTP_HANDLER_THREADS]
      ->message_loop()
      .PostWork(m_CallbackFactory.NewCallback(&MoonlightInstance::STUNCallback,
                                              callbackId, args));
}

void MoonlightInstance::STUNCallback(int32_t /*result*/, int32_t callbackId,
                                     pp::VarArray args) {
...
namespace pp {
Module* CreateModule() { return new MoonlightModule(); }
}  // namespace pp
