...
static bool s_DecoderErrorSent;
...
bool MoonlightInstance::InitializeRenderingSurface(int width, int height) {
  ...
  int32_t contextAttributes[] = {PP_GRAPHICS3DATTRIB_ALPHA_SIZE,
                                 8,
  ...
  int32_t swapBehaviorAttribute[] = {PP_GRAPHICS3DATTRIB_SWAP_BEHAVIOR,
...
int MoonlightInstance::VidDecSetup(int videoFormat, int width, int height,
  ...
  if (!(drFlags & DR_FLAG_FORCE_SW_DECODE)) {
    ...
    err = g_Instance->m_VideoDecoder->Initialize(
        g_Instance->m_Graphics3D, PP_VIDEOPROFILE_H264HIGH,
        PP_HARDWAREACCELERATION_ONLY, 0, pp::BlockUntilComplete());
  } else {
  ...
  if (err == PP_ERROR_NOTSUPPORTED) {
    ...
    err = g_Instance->m_VideoDecoder->Initialize(
        g_Instance->m_Graphics3D, PP_VIDEOPROFILE_H264HIGH,
        PP_HARDWAREACCELERATION_NONE, 0, pp::BlockUntilComplete());
    ...
    if (err == PP_ERROR_NOTSUPPORTED) {
      ...
      ClDisplayTransientMessage(
          "Hardware decoding is unavailable. Falling back to CPU decoding");
    }
  }

  pp::Module::Get()->core()->CallOnMainThread(
      0, g_Instance->m_CallbackFactory.NewCallback(
             &MoonlightInstance::DispatchGetPicture));
...
void MoonlightInstance::DispatchGetPicture(uint32_t unused) {
  ...
  g_Instance->m_VideoDecoder->GetPicture(
      g_Instance->m_CallbackFactory.NewCallbackWithOutput(
          &MoonlightInstance::PictureReady));
}
...
static void WriteSpsNalu(PLENTRY nalu, unsigned char* outBuffer,
                         unsigned int* offset) {
  int start_len = nalu->data[2] == 0x01 ? 3 : 4;
  h264_stream_t* stream = h264_new();

  // Read the old NALU
  read_nal_unit(stream, (unsigned char*)&nalu->data[start_len],
  ...
  *offset += write_nal_unit(stream, &outBuffer[*offset],
                            nalu->length + 32 - start_len);
...
int MoonlightInstance::VidDecSubmitDecodeUnit(PDECODE_UNIT decodeUnit) {
  ...
  int32_t err;
  if (!s_DecoderErrorSent && (err = g_Instance->m_VideoDecoder->Decode(
                                  packedMillis, offset, s_DecodeBuffer,
                                  pp::BlockUntilComplete())) != PP_OK) {
    char buffer[200];
    snprintf(buffer, sizeof(buffer),
             "Decoder can't process images. Error: %d. Please try a lower "
             "resolution",
             err);
    ClDisplayTransientMessage(buffer);
    s_DecoderErrorSent = true;
  }
...
Shader MoonlightInstance::CreateProgram(const char* vertexShader,
                                        const char* fragmentShader) {
  Shader shader;

  shader.program = glCreateProgram();
  CreateShader(shader.program, GL_VERTEX_SHADER, vertexShader,
               strlen(vertexShader));
  CreateShader(shader.program, GL_FRAGMENT_SHADER, fragmentShader,
               strlen(fragmentShader));
  ...
  shader.texcoord_scale_location =
      glGetUniformLocation(shader.program, "v_scale");
...
void MoonlightInstance::PaintPicture(void) {
  ...
  if (m_CurrentPicture.texture_target != s_LastTextureType) {
    if (m_CurrentPicture.texture_target == GL_TEXTURE_2D) {
      if (!g_Instance->m_Texture2DShader.program) {
        g_Instance->m_Texture2DShader =
            CreateProgram(k_VertexShader, k_FragmentShader2D);
      }
      glUseProgram(g_Instance->m_Texture2DShader.program);
      glUniform2f(g_Instance->m_Texture2DShader.texcoord_scale_location, 1.0,
                  1.0);
    } else if (m_CurrentPicture.texture_target == GL_TEXTURE_RECTANGLE_ARB) {
      if (!g_Instance->m_RectangleArbShader.program) {
        g_Instance->m_RectangleArbShader =
            CreateProgram(k_VertexShader, k_FragmentShaderRectangle);
      }
      glUseProgram(g_Instance->m_RectangleArbShader.program);
      glUniform2f(g_Instance->m_RectangleArbShader.texcoord_scale_location,
                  m_CurrentPicture.texture_size.width,
                  m_CurrentPicture.texture_size.height);
    } else if (m_CurrentPicture.texture_target == GL_TEXTURE_EXTERNAL_OES) {
      if (!g_Instance->m_ExternalOesShader.program) {
        g_Instance->m_ExternalOesShader =
            CreateProgram(k_VertexShader, k_FragmentShaderExternal);
      }
      glUseProgram(g_Instance->m_ExternalOesShader.program);
      glUniform2f(g_Instance->m_ExternalOesShader.texcoord_scale_location, 1.0,
                  1.0);
    }
  ...
  if (m_CurrentPicture.texture_id != s_LastTextureId ||
      m_CurrentPicture.texture_target != originalTextureTarget) {
